...
export default function ProjectHistoryTab({ projectId }) {
  ...
  const load = useCallback(async (reset, currentLength) => {
  ...
  }, [tableFilter, projectId]);

  useEffect(() => { load(true, 0); }, [tableFilter, projectId]); // eslint-disable-line react-hooks/exhaustive-deps

  function handleLoadMore() {
    setLoadingMore(true);
    load(false, entries.length);
  }
  ...
  return (
    <div className="project-history-tab">
      ...
      {!entries.length ? (
        <p className="version-column-hint">Nenhum registro ainda pra esse projeto.</p>
      ) : (
